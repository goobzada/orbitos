...
class StatsController {
    ...
    async getOverview(req, res) {
        ...
        try {
            ...
            const userOrgs = await prisma_1.default.organization.findMany({
                where: {
                    OR: [
                        { ownerId: userId },
                        { members: { some: { userId } } }
            ...
            const activeServers = await prisma_1.default.server.count({
                where: {
            ...
            const openTickets = await prisma_1.default.ticket.count({
                where: {
                    organizationId: { in: orgIds },
                    status: 'OPEN'
                }
            ...
            const totalStaff = await prisma_1.default.staffMember.count({
                where: {
            ...
            const revenue24h = await prisma_1.default.storeOrder.aggregate({
                where: {
                    organizationId: { in: orgIds },
                    status: 'PAID',
                    paidAt: { gte: last24h }
            ...
            const orders = await prisma_1.default.storeOrder.findMany({
                where: {
                    organizationId: { in: orgIds },
                    status: 'PAID',
