import { Interaction, ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder, ButtonInteraction, ModalSubmitInteraction, EmbedBuilder } from 'discord.js';
import { allowlistClient, AllowlistQuestion } from './allowlist-api-client';
...
export async function handleAllowlistInteraction(interaction: Interaction) {
...
async function handleStartButton(interaction: ButtonInteraction) {
    ...
    try {
        const activeFormRes = await allowlistClient.getActiveForm(interaction.guildId);
        if (!activeFormRes) {
        ...
        log.error(`[AllowlistFlow] Erro ao iniciar form: ${err.message}`);
...
async function handleModalSubmit(interaction: ModalSubmitInteraction) {
    ...
    try {
        ...
        const result = await allowlistClient.submitAllowlist(formId, payload);
        ...
        log.error(`[AllowlistFlow] Submissão Modal: ${err.message}`);
        activeSessions.delete(interaction.user.id);
